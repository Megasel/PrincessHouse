// dnSpy decompiler from Assembly-CSharp.dll class: SkinnedMeshCombiner
using System;
	...
	private void Start()
	{
		...
		for (int j = 0; j < this.m_SpriteMeshInstances.Length; j++)
		{
			...
			if (spriteMeshInstance2.cachedSkinnedRenderer)
			{
				...
				foreach (BoneWeight boneWeight in spriteMeshInstance2.sharedMesh.boneWeights)
				{
				...
				foreach (Transform item2 in cachedSkinnedRenderer.bones)
				{
					list.Add(item2);
				}
