// dnSpy decompiler from Assembly-CSharp.dll class: Grid_button
using System;
using System.Collections;
using UnityEngine;

public class Grid_button : MonoBehaviour
{
	private void Start()
	{
		if (Grid_button._inst == null)
		{
			Grid_button._inst = this;
		}
	}
	...
	private IEnumerator shower_Btn()
	{
		...
		Room_Cleaning_Main._inst.hand_shower_g.SetActive(false);
	...
	private IEnumerator Dust_Remover_Btn()
	{
		...
		Room_Cleaning_Main._inst.hand_dust_remover_g.SetActive(false);
		Room_Cleaning_Main._inst.hand_green_table.SetActive(true);
		Room_Cleaning_Main._inst.green_mud_sm.SetActive(true);
		iTween.MoveTo(this.Bg, iTween.Hash(new object[]
		{
			"x",
			3.6f,
	...
	private IEnumerator Mud_Remover_carpet()
	{
		...
		Room_Cleaning_Main._inst.hand_mud_carpet_g.SetActive(false);
		iTween.MoveTo(this.Bg, iTween.Hash(new object[]
		{
			"x",
			-2.55f,
		...
		yield return new WaitForSeconds(1.5f);
		Room_Cleaning_Main._inst.carpet_mud_sm.SetActive(true);
		Room_Cleaning_Main._inst.hand_mud_carpet.SetActive(true);
	...
	private IEnumerator Spider_Boom()
	{
		...
		Room_Cleaning_Main._inst.hand_spider_g.SetActive(false);
		iTween.MoveTo(this.Bg, iTween.Hash(new object[]
		{
			"x",
			-5.59f,
		...
		yield return new WaitForSeconds(1.5f);
		Room_Cleaning_Main._inst.hand_spider.SetActive(true);
		Room_Cleaning_Main._inst.spider_sm.SetActive(true);
	...
	private IEnumerator Water_Viper()
	{
		...
		Room_Cleaning_Main._inst.hand_water_g.SetActive(false);
		iTween.MoveTo(this.Bg, iTween.Hash(new object[]
		{
			"x",
			-3.85f,
		...
		yield return new WaitForSeconds(1.5f);
		Room_Cleaning_Main._inst.hand_pink_water.SetActive(true);
		Room_Cleaning_Main._inst.water_pink_sm.SetActive(true);
		Task_Bar._inst.bar_pink_water.SetActive(true);
		SoundManager.Instance.v_o_viper();
		yield break;
	}

	public static Grid_button _inst;
