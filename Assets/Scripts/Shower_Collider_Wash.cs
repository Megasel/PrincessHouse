// dnSpy decompiler from Assembly-CSharp.dll class: Shower_Collider_Wash
using System;
using System.Collections;
using UnityEngine;

public class Shower_Collider_Wash : MonoBehaviour
{
	private void Start()
	{
	}
	...
	private IEnumerator OnTriggerEnter(Collider col)
	{
		...
		if (base.gameObject.name == "shower_coll" && col.gameObject.tag == "mirror_bubble")
		{
			...
			if (this.count == 6)
			{
				...
				SoundManager.Instance.v_o_spounge();
			}
		}
		if (base.gameObject.name == "shelf_cleaner_coll" && col.gameObject.tag == "mask_bubble")
		{
			...
			UnityEngine.Debug.Log(this.fill);
			if (!base.GetComponent<AudioSource>().isPlaying)
			{
				base.GetComponent<AudioSource>().Play();
			}
			if (this.count1 == 12)
			{
				...
				WashRoom_Main._inst.hand_dust_remover_g.SetActive(true);
				Grid_button_Wash._inst.grid_btn[1].enabled = true;
			}
		}
		yield break;
	}
