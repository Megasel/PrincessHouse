// dnSpy decompiler from Assembly-CSharp.dll class: Shower_Collider_Kitchen
using System;
using System.Collections;
using UnityEngine;

public class Shower_Collider_Kitchen : MonoBehaviour
{
	private void Start()
	{
	}
	...
	private IEnumerator OnTriggerEnter(Collider col)
	{
		...
		if (base.gameObject.name == "shower_coll" && col.gameObject.tag == "mirror_bubble")
		{
			...
			if (this.count == 6)
			{
				...
				Kitchen_Main._inst.hand_window.SetActive(false);
				...
				Kitchen_Main._inst.hand_window.SetActive(true);
		...
		if (base.gameObject.name == "shelf_cleaner_coll" && col.gameObject.tag == "mask_bubble")
		{
			...
			if (this.count1 == 12)
			{
				...
				Kitchen_Main._inst.hand_dust_remover_g.SetActive(true);
				Grid_button_Kitchen._inst.grid_btn[1].enabled = true;
			}
		}
		yield break;
	}
