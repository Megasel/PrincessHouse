// dnSpy decompiler from Assembly-CSharp.dll class: Dust_Remover_Collider_Wash
using System;
using System.Collections;
using UnityEngine;

public class Dust_Remover_Collider_Wash : MonoBehaviour
{
	private void Start()
	{
	}
	...
	private IEnumerator OnTriggerEnter(Collider col)
	{
		...
		if (base.gameObject.name == "dust_remover_coll" && col.gameObject.tag == "mud_mask_green")
		{
			...
			if (this.count == 12)
			{
				...
				WashRoom_Main._inst.hand_green_table.SetActive(false);
				WashRoom_Main._inst.hand_purple_table.SetActive(true);
				...
				WashRoom_Main._inst.purple_mud_sm.SetActive(true);
		...
		if (base.gameObject.name == "dust_remover_coll" && col.gameObject.tag == "mud_mask_purple")
		{
			...
			if (this.count == 21)
			{
				...
				WashRoom_Main._inst.hand_purple_table.SetActive(false);
				WashRoom_Main._inst.hand_pink_table.SetActive(true);
				iTween.MoveTo(this.drag_tool, iTween.Hash(new object[]
				{
					"x",
					3.95f,
				...
				yield return new WaitForSeconds(1f);
				WashRoom_Main._inst.pink_mud_sm.SetActive(true);
		...
		if (base.gameObject.name == "dust_remover_coll" && col.gameObject.tag == "mud_mask_pink")
		{
			...
			if (this.count == 6)
			{
				...
				WashRoom_Main._inst.hand_pink_table.SetActive(false);
				iTween.MoveTo(this.drag_tool, iTween.Hash(new object[]
				{
					"x",
					10.568161f,
				...
				yield return new WaitForSeconds(1f);
				WashRoom_Main._inst.hand_mud_carpet_g.SetActive(true);
				Grid_button_Wash._inst.grid_btn[2].enabled = true;
			}
		}
		yield break;
	}
